...
namespace ProjetGED.Controllers
{
    public class FolderController : Controller
    {
        ...
        [HttpPost]
        [Route("Folder/Rename")]
        public ActionResult Rename(string folderId, string newFolderName)
        {

            using (var context = new GEDContext())
            {
                var fId = Int16.Parse(folderId);
                var folder = context.Folders.Where(u => u.Id == fId).FirstOrDefault();
        ...
        [HttpPost]
        [Route("Folder/DocumentRename")]
        public ActionResult DocumentRename(string documentId, string newDocumentName)
        {
        ...
        [HttpPost]
        [Route("Folder/Remove")]
        public ActionResult Remove(string folderId)
        {
            using(var context = new GEDContext())
            {
                var folId = Int16.Parse(folderId);
                var folder = context.Folders.Include("Documents").First();
        ...
        [Route("Folder/Shared/{folder?}")]
        public ActionResult Shared(string folder = null)
        {
            var id = this.UserId();
            ...
            else
            {
                ...
                using (var context = new GEDContext())
                {
                    ...
                    Folder folderParentID = context.Folders.Where(f => f.Name == folder).FirstOrDefault();

                    var accessFolder = context.Folders.ToList().FindAll(f => f.Parent == folderParentID);
                    

                    var accessDocument = context.Documents.ToList().FindAll(f => f.Folder == folderParentID);
